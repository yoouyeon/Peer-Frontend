...
export const handlers = [
  ...
  http.get(`${API_PATH.team.comment}/:postId`, ({ params, request }) => {
    ...
    // 댓글 반환 (공지사항 & 게시판)
    const comments = mockPostDataStore.getComments(numberPostId)
    if (comments) {
  ...
  http.put<PutCommentParam, PutCommentBody, never>(
    ...
    async ({ params, request }) => {
      ...
      // 댓글 수정 (공지사항 & 게시판)
      const isSuccess = mockPostDataStore.updateComment(
        numberCommentId,
        content,
      )
      if (!isSuccess) {
  ...
  http.delete<DeleteCommentParam, never, never>(
    ...
    ({ params, request }) => {
      ...
      // 댓글 삭제 (공지사항 & 게시판)
      const isSuccess = mockPostDataStore.deleteComment(Number(commentId))
      if (isSuccess) {
        return HttpResponse.json(
          { message: '댓글이 삭제되었습니다.' },
          { status: HTTP_STATUS.ok },
  ...
  http.put<PutPostParam, PutPostBody, null | ErrorResponse>(
    ...
    async ({ params, request }) => {
      ...
      // 게시글 수정
      const isSuccess = mockPostDataStore.updatePost(numberPostId, {
        title,
        content,
      })
      if (!isSuccess) {
  ...
  http.delete<DeletePostParam, never, null | ErrorResponse>(
    ...
    ({ params, request }) => {
      ...
      const numberPostId = Number(postId)
      // 게시글 삭제
      const isSuccess = mockPostDataStore.deletePost(numberPostId)
      if (!isSuccess) {
        return HttpResponse.json(
          { message: '게시글을 찾을 수 없습니다.' },
          { status: HTTP_STATUS.notFound },
